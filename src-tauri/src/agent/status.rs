...
/// `SubagentStarted`/`SubagentStopped` track in-flight *background* work:
...
/// deliberately *not* gated here: they have an increment-side signal
...
/// in-flight background subagents. While background work is in flight,
...
mod tests {
    ...
    fn apply_event_emits_on_real_transition() {
        ...
        // A Working event after NeedsAttention is a real change → emitted.
        assert_eq!(apply_event(&mut states, "a1", StatusEvent::Working), Some(AgentRunState::Working));
    }
    ...
    fn needs_attention_to_working_is_reachable() {
        // After a permission prompt, a Working event (PreToolUse/UserPromptSubmit)
    ...
    // ── display_state matrix: counter → pill ──────────────────────────────────
