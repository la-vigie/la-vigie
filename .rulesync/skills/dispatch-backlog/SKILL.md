...
description: Scan a project's low-priority todo backlog, pick a small set of genuine one-shot tickets, classify each for GUI-review need, dispatch them as La Vigie tasks, and wire the right completion chain per class. Conservative by design — errs toward NOT dispatching.
targets: ["*"]
claudecode:
  allowed-tools: Bash, Read, Skill, ToolSearch
  argument-hint: '[project/repo] [--priority low] [--cap 3]'
---
