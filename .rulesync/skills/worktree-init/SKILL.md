---
name: worktree-init
description: Initialize a worktree session — read the task via the repo's task-provider (or the launch prompt if none), rename this La Vigie task, set it in progress, then take the fast path (one-sentence diffs) or a full spec.
targets: ["*"]
claudecode:
